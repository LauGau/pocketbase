...
migrate((app) => {
  ...
  unmarshal({
    "createRule": "// is user logged in ?\n@request.auth.id != \"\" &&\n\n@collection.collectionMembers.pinCollection ?= @request.body.pinCollection.id && @collection.collectionMembers.user ?= @request.auth.id && @collection.collectionMembers.status ?= \"approved\""
  }, collection)
