...
routerAdd('GET', '/pbk/test', (e) => {
    ...
    try {
        ...
        $app.logger().error(
            'Failed to parse URL in /pbk/test',
            'url',
            urlTarget,
            'error',
            error
...
routerAdd(
    ...
    (c) => {
        ...
        const isArchive = requestData.query['isArchive']
            ? JSON.parse(requestData.query['isArchive'])
            : false;
        ...
        function handleDomainScopeMatch(c, filter, page, perPage, sort) {
            ...
            try {
                ...
                const allItems = c.app.findRecordsByFilter(
                    ...
                    sort,
                    0,
                    0 // Fetch all to get the total count
                );
                ...
                $app.logger().error(
                    "Database query failed for 'domain' scope",
                    'filter',
                    filter,
                    'error',
                    dbError
        ...
        function handlePageScopeMatch(
            ...
            try {
                ...
                $app.logger().error(
                    "Database query failed for 'page' scope",
                    'filter',
                    filter,
                    'error',
                    dbError
            ...
            for (const pin of initialRecords) {
                let urlMatchingData;
                try {
                    urlMatchingData = JSON.parse(
                        pin.get('urlMatching') || '{}'
                    );
                } catch (parseError) {
                    $app.logger().warn(
                        `Skipping pin with invalid urlMatching JSON`,
                        'pinId',
                        pin.id,
                        'error',
                        parseError
                ...
                if (
                    ...
                    for (const pattern of urlMatchingData.patterns) {
                        try {
                            ...
                            $app.logger().error(
                                `Invalid pattern for pin ${pin.id}: "${pattern}"`,
                                'error',
                                e
                            );
                        }
        ...
        const domainConditions = domainKeys.map(
            (key) => `urlMatching.domains ?~ "${key}"`
        );
